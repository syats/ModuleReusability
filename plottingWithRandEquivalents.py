...
# For each
def plotPerMatrixSeveralCurves(allStores, namesOfTypes=['REAL', 'DP-Rand', 'RSS-Rand', 'RSS2'], namesOfData=[],
                               numPerType={'REAL': 1, 'RAND': 1, 'RSS3': 1}):
    fileNames = allStores.keys()
    for fn in fileNames:
        plt.figure()


def plotWithReasAsBaes(allData, typesOfData, Cs, namesOfData=[], namesOfTypes=['REAL', 'DP-Rand', 'RSS-Rand', 'RSS2'],
                       randToConsiderForSums=[1, 2], markersOfTypes=['*', 'o', 'v', 'o'],
                       colorsOfTypes=['r', 'b', 'g', 'k']):
...
def makePlots(allData, typesOfData, Cs, namesOfData=[], namesOfTypes=['REAL', 'DP-Rand', 'RSS-Rand', 'RSS2'],
              randToConsiderForSums=[1, 2], markersOfTypes=['*', 'o', 'v', 'o'], colorsOfTypes=['r', 'b', 'g', 'k'],
              fig1FName='', fig2FName='', plotKvsData=True, plotDataSums=True, sprows=2, spcols=4,
              plotTheHistograms=True, avgY=False, logX=True, logY=False, QCs=[]):
    fileNames = allData.keys()
    ...
    if avgY:
        ...
        totalMax = max(
            [max([cur.max() for cur in allData[fn]]) / (min(Cs[fn].shape) * max(Cs[fn].shape)) for fn in fileNames])
    ...
    if plotKvsData or (not (fig2FName == '')):
        ...
        for fn in fileNames:
            ...
            for dataNum in range(len(allData[fn])):
                ...
                X = np.array(range(Cs[fn].shape[1], int(len(data) / portion)))
                Y = data[X]
                if avgY:
                    Y = Y / (X)
                    Y = Y / (float(m))
        ...
        if (len(fileNames) > 1):
            h, l = ax.get_legend_handles_labels()
            fig2.legend(loc=1, ncol=1, borderaxespad=0., labels=namesOfTypes, handles=h[:len(namesOfTypes)],
                        numpoints=1)
...
# --------- B A N D S ---------
def makeBands(allData, typesOfData, Cs, namesOfData=[], namesOfTypes=['REAL', 'DP-Rand', 'RSS-Rand', 'RSS2'],
              randToConsiderForSums=[1, 2], markersOfTypes=['*', 'o', 'v', 'o'], colorsOfTypes=['r', 'b', 'g', 'k'],
              fig1FName='', fig2FName='', plotKvsData=True, plotDataSums=True, avgY=False, logX=False, logY=False,
              QCs=[], numStds=2, theAx=None, yLabel=None, legendPos=0, finalK=None,
              lightColors=['#ff6666', '#6666ff', '#66ff66'], **kwargs):
    fileNames = allData.keys()
    totalMax = max([max([deco.max() for deco in fnl]) for fnl in allData.values()])
    totalMin = min([min([deco.min() for deco in fnl]) for fnl in allData.values()])
    print("totals " + str(totalMin) + " -> " + str(totalMax))
                ...
                if not (dataType in placedLegends):
                    if (ii == 0):
                        ladded = ax.plot(XX, YY1, colorsOfTypes[ii] + extraStyle, label=namesOfTypes[ii],
                                         markeredgewidth=0.0, zorder=100, fillstyle='full', linewidth=2)
                        print("yy1min" + str(min(YY1)))
                        placedLines.append(ladded[0])
                    else:
                        # STDs
                        ax.fill_between(XX, YY1a, YY2a, facecolor=colorsOfTypes[ii], edgecolor=colorsOfTypes[ii],
                                        zorder=20, interpolate=False, label=namesOfTypes[ii])
                        ladded = plt.Rectangle((0, 0), 1, 1, fc=colorsOfTypes[ii])
                        # MIN MAX
                        ax.fill_between(XX, YY1b, YY2b, facecolor=lightColors[ii], edgecolor=lightColors[ii], zorder=1,
                                        interpolate=False)
                        ladded = plt.Rectangle((0, 0), 1, 1, fc=colorsOfTypes[ii])
                        placedLines.append(ladded)
                    # mean
                    ...
                    if (ii == 0):
                        ...
                        ladded = ax.fill_between(XX, YY1, YY2, facecolor=colorsOfTypes[ii], edgecolor=colorsOfTypes[ii],
                                                 alpha=0.8, interpolate=False, label=namesOfTypes[dataType])
